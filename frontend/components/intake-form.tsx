...
import { defaultHomePage, type CmsIntake } from "@/lib/cms/site"
...
export function IntakeForm({
  content = defaultHomePage.intake,
}: {
  content?: CmsIntake
}) {
  return (
    ...
    >
      <div className="mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-start lg:px-8">
        ...
        <IntakeFormContent
          content={content}
          className="rounded-[22px] bg-white p-6 text-foreground shadow-[0_28px_64px_-36px_rgba(8,28,21,0.45)] ring-1 ring-black/5 sm:p-8"
        />
      </div>
    </section>
  )
...
export function IntakeFormContent({
  ...
  const serviceOptions = content.needOptions
  const regionOptions = content.regionOptions
  const powerOptions = content.footprintOptions
  const timelineOptions = content.timelineOptions
